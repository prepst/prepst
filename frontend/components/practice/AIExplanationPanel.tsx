...
import { X, Pin, ChevronRight, ChevronDown, ArrowUp, Square } from "lucide-react";
...
export function AIExplanationPanel({
    ...
    const [explanationMode, setExplanationMode] = useState<"step" | "full">("step");
...
`;
            const questionContextStr = questionContext?.stem
                ? `Question: "${questionContext.stem}"${questionContext.topic ? ` (Topic: ${questionContext.topic})` : ''}\n\n`
                : '';
        ...
        <>
            ...
            <div
                className={cn(
                    ...
                    "bg-background/95 dark:bg-[#1a1a2e]/95 backdrop-blur-xl border-l border-border",
                        ...
                        <div className="relative">
                            ...
                            {showModeDropdown && (
                                <div className="absolute top-full left-0 mt-1 bg-popover border border-border rounded-lg shadow-lg py-1 z-50 min-w-[140px]">
                                    <button
                                        onClick={() => { setChatMode("answer"); setShowModeDropdown(false); }}
                                        className={cn(
                                    ...
                                    <button
                                        onClick={() => { setChatMode("tutor"); setShowModeDropdown(false); }}
                                        className={cn(
                                            "w-full px-3 py-2 text-left text-sm hover:bg-muted transition-colors",
                            ...
                            <div className="flex flex-col items-center justify-center h-full">
                                <div className="w-20 h-20 mb-6">
                                    <svg viewBox="0 0 100 100" className="w-full h-full">
                                        <defs>
                                            <linearGradient id="pigGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                                                <stop offset="0%" stopColor="#FFB6C1" />
                                                <stop offset="100%" stopColor="#FF69B4" />
                                            </linearGradient>
                                        </defs>
                                        {/* Ears */}
                                        <ellipse cx="25" cy="25" rx="12" ry="15" fill="url(#pigGradient)" transform="rotate(-20 25 25)" />
                                        <ellipse cx="75" cy="25" rx="12" ry="15" fill="url(#pigGradient)" transform="rotate(20 75 25)" />
                                        <ellipse cx="25" cy="25" rx="6" ry="8" fill="#FF1493" transform="rotate(-20 25 25)" />
                                        <ellipse cx="75" cy="25" rx="6" ry="8" fill="#FF1493" transform="rotate(20 75 25)" />
                                        {/* Face */}
                                        ...
                                        {/* Blush */}
                                        <circle cx="25" cy="58" r="6" fill="#FF69B4" opacity="0.5" />
                                        <circle cx="75" cy="58" r="6" fill="#FF69B4" opacity="0.5" />
                                        {/* Smile */}
                                        <path d="M 40 75 Q 50 82 60 75" stroke="#FF1493" strokeWidth="2" fill="none" strokeLinecap="round" />
                                    </svg>
                                </div>

                                <h2 className="text-2xl font-bold text-foreground mb-2">How can I help?</h2>
                                <p className="text-muted-foreground text-sm mb-8">Ask Peppa any question!</p>
                                ...
                                <div className="flex flex-col items-center gap-2 text-sm">
                                    <div className="flex items-center gap-2 px-4 py-2 bg-muted rounded-full border border-border">
                                        ...
                                        <span className="text-foreground">
                                            You have <span className="text-cyan-500 font-semibold">24</span> orbs remaining
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <ul className="space-y-2">
                                        {currentStepData.bullets.map((bullet, i) => (
                                            <li key={i} className="flex items-center gap-2 text-foreground">
                                                ...
                                                <span dangerouslySetInnerHTML={{ __html: bullet.replace(/x/g, '<em>x</em>') }} />
                                            </li>
                                        ))}
                                    </ul>
                                )}
                        ...
                        <div className="space-y-6">
                            {questionContext?.rationale ? (
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{ __html: questionContext.rationale }}
                                />
                            ) : (
                                <div className="space-y-6">
                                    {steps.map((step, i) => (
                                        <div key={i} className="space-y-3">
                                            ...
                                            <p className="text-muted-foreground ml-6">{step.content}</p>
                                            {step.bullets && (
                    ...
                    {activeTab === "ask" ? (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-muted-foreground text-sm">Got a question? Ask Peppa Chat</span>
                            </div>
