"""
Disable Questions with PNG Images in Question Stems

This script identifies and disables all questions that have PNG images
embedded in their question stems (the question text itself).

Usage:
    # Dry run (show what would be disabled)
    python scripts/disable_png_stem_questions.py --dry-run

    # Actually disable the questions
    python scripts/disable_png_stem_questions.py --execute
"""
...
def has_png_in_stem(stem):
    """
    Check if stem contains PNG images

    Args:
        stem: Question text/stem field

    Returns:
        bool: True if PNG images found in stem
    """
    if not stem:
        return False
...
def find_png_stem_questions():
    """
    Find all questions with PNG images in their stems

    Returns:
        list: List of question dictionaries with PNG in stems
    """
    print("🔍 Scanning database for questions with PNG in question stems...")
    ...
    while True:
        result = supabase.table('questions').select(
            'id, external_id, stem, module, difficulty, is_active'
    ...
    # Filter questions with PNG in stems
    png_questions = []
    for q in all_questions:
        if has_png_in_stem(q.get('stem')):
...
def main():
    parser = argparse.ArgumentParser(
        description='Disable questions with PNG images in question stems'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    # Find questions with PNG in stems
    png_questions = find_png_stem_questions()

    print(f"\n📊 Found {len(png_questions)} questions with PNG in question stems")

    if len(png_questions) == 0:
        print("✅ No questions found with PNG images in stems. Nothing to do!")
        return
    ...
    if len(active_png_questions) == 0:
        print("\n✅ All PNG stem questions are already disabled!")
        return
    ...
    if args.dry_run:
        ...
        print(f"\n✅ DONE! Disabled {disabled_count} questions with PNG in stems")
        print(f"   {inactive_count} were already inactive")
        print(f"   Total PNG stem questions in database: {len(png_questions)}")
