...
10. IMPORTANT - Spatial positioning: Use .next_to(obj, direction, buff=0.5) for relative positioning, .to_edge(direction) for screen edges, .to_corner(UL/UR/DL/DR) for corners. Always use buff parameter to control spacing and avoid crowding.
11. CRITICAL - Frame boundaries: ALL elements must stay within the visible frame. Scale down text/objects if needed to fit. Standard text sizes: title=1.5, subtitle=1.0, body=0.8. Use .scale() to ensure nothing is cut off.
12. CRITICAL - Text management: Never overlap text. Use self.play(FadeOut(old_text)) to remove previous text before adding new text. Use VGroup() to group related elements for easier positioning and removal. Keep the scene clean.
